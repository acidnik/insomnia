...
- Стейт — по JSON-файлу на проверку, переживает рестарт демона. Хранит `last_run_at`: рестарт НЕ запускает проверки — запускаются только просроченные (period < now - last_run_at); правки периода применяются от момента последнего прогона. Пишется дважды за прогон: перед запуском (`last_run_at` для расписания) и после (исход прогона: `alert_active`/`failing_since`/repeat). Без второй записи снапшот отстаёт на один исход, и рестарт воскрешает уже отданный `restored` (переотправка с растущим downtime) или теряет свежий алерт.
